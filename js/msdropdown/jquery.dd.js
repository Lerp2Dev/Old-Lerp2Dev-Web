...
function dd(element, settings) {
	...
	var _parseOption = function(opt) {
		var imagePath = '', title ='', description='', value=-1, text='', className='', imagecss = '', lang = '';
		if (opt !== undefined) {
			...
			lang = $(opt).prop("data-lang") || $(opt).data("lang") || (lang || "");

		};
		var o = {image: imagePath, title: title, description: description, value: value, text: text, className: className, imagecss:imagecss, lang: lang};
		return o;
	};	 
	...
	var _createChild = function (opt) {
		...
		var link = _doc.createElement("div");
		link.onclick = function() {window.location = _insertParam("lang="+parsed.lang)};
		li.appendChild(link);
		if (oIcon) {
			link.appendChild(oIcon);
		};
		link.appendChild(oTitleText);
		if (oDescription) {
			link.appendChild(oDescription);
		} else {
			if (oIcon) {
...
function selectIndex(elem) 
{
    ...
    if(typeof query.lang !== "undefined") 
    {
        ...
        {
            if(elem[i].getAttribute("data-lang") == lang) 
            {
                index = i;
                break;
            }
